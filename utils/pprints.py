...
class PPrints:
    ...
    def print_with_lock(self, query: str, status: str, mode: str,
        ...
        with self._print_lock:
            ...
            print(f"+--------------------------------------------------+\n"
                  f"Scrapping DATA from Google Maps....\n"
                  f"[+] Developer: @imzpider (emma_luv)\n"
                  f"[+] Query: {query}\n[+] {results_index_data}\n[+] Status: {status}\n"
                  f"{self.RED}[+] MemoryUsage: {current_memory_usage: .2f}MB\n"
                  f"[+] {self.RED}Note: Don't open the file until the script finish!\n"
                  f"{self.RESET}+--------------------------------------------------+\n", end="\r")
